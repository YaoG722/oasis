...
if __name__ == "__main__":
    ...
    for i, gpu in enumerate(gpus):
        cmd = (
            ...
            f"--served-model-name 'Qwen2.5-14B' "  # 对外暴露的模型名称
            f"--tensor-parallel-size 2 "  # GPU 并行数，单卡部署需关闭
            f"--host {host} --port {ports[i]} "  # 绑定IP和端口
            f"--gpu-memory-utilization 0.7 "  # 设置GPU显存利用率阈值 (0.9)
            f"--max-model-len 32768 "  # 设置模型最大上下文长度 (max: 32768)
            # f"--enable-chunked-prefill "  # 启用分块预填充
            # f"--max-num-batched-tokens 2048 "  # 最大批处理令牌数 (512)
            f"--max-num-seqs 64 "  # 最大序列数 (256)
